...
1.  **Initial Setup**: Install dependencies and run the one-time library scan.
2.  **Add New Comics**: Place new, unsorted comic files (`.cbz`, `.cbr`, `.pdf`) into the `inbox` folder.
3.  **Process Inbox**: Run the main `process_inbox.py` script. It will automatically convert, tag, check for duplicates, and file everything it can.
4.  **Manage Quarantine**: For any files the script couldn't handle (e.g., ambiguous names, true duplicates), use the `manage_quarantine.py` script to easily fix, delete, or re-process them.
